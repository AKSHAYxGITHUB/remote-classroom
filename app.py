...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'rajasthan-digi-shala-2025'
...
def dashboard():
    ...
    if current_user.role == 'teacher':
        ...
        # For teachers, available_courses can be an empty list
        available_courses = []

    else: # This is the student part that we are modifying
        # Get courses the student is already enrolled in
        enrolled_courses = conn.execute('''
            SELECT c.*, u.username as teacher_name,
            (SELECT COUNT(*) FROM attendance WHERE student_id = ? AND course_id = c.id AND status = 'present') as present_count,
            (SELECT COUNT(*) FROM attendance WHERE student_id = ? AND course_id = c.id) as total_attendance
            FROM courses c 
        ...
        # Rename enrolled_courses to courses for the template
...
def create_course():
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO courses (title, description, teacher_id) VALUES (?, ?, ?)',
...
def course(course_id):
    ...
    if current_user.role == 'student':
        enrollment = conn.execute('SELECT * FROM enrollment WHERE user_id = ? AND course_id = ?',
    ...
    materials = conn.execute('SELECT * FROM materials WHERE course_id = ?', (course_id,)).fetchall()
    ...
    conn.close()
    return render_template('course.html', course=course, materials=materials, posts=posts, students=students)
...
def upload_material(course_id):
    ...
    if file:
        ...
        conn = get_db_connection()
        conn.execute('INSERT INTO materials (course_id, title, filepath) VALUES (?, ?, ?)',
...
def enroll(course_id):
    ...
    existing = conn.execute('SELECT * FROM enrollment WHERE user_id = ? AND course_id = ?',
                           (current_user.id, course_id)).fetchone()
    if existing:
        flash('You are already enrolled in this course')
    else:
        conn.execute('INSERT INTO enrollment (user_id, course_id) VALUES (?, ?)',
...
def take_attendance(course_id):
    ...
    conn.execute('DELETE FROM attendance WHERE course_id = ? AND date = ?',
...
def ask_ai():
    ...
    context = f"Course materials context: "
    for material in materials:
        context += f"{material['title']} "
...
def post_question(course_id):
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO posts (course_id, user_id, content, timestamp) VALUES (?, ?, ?, ?)',
...
def post_reply(post_id):
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO replies (post_id, user_id, content, timestamp) VALUES (?, ?, ?, ?)',
